...
-- ── reward_request (ごほうび申請) ────────────────────────────────────────
-- A child requests a reward exchange; a parent approves/rejects it on their
-- own device. reward_id intentionally has no FK to reward(id) so the request
-- history survives the reward later being edited or deleted. point_history_id
-- links an approved request to the point_history row it produced. notified_at
-- is unused for now, reserved for a future push notification pass so it
-- never re-notifies for the same request.
...
create policy "parent manages own reward_request" on reward_request
